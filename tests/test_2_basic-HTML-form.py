...
def test_example_page_title(page):
    common.check_page_title(page, "HTML Form Elements")
...
def test_fill_username(page):
    ...
    # Check if the username field is focused
    is_focused = page.evaluate(
        "document.activeElement === document.querySelector('input[name=\"username\"]')"
    )
    assert is_focused, "Expected the username input field to be focused after clicking."
